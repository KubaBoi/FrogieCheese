...
function setSeenMessage(chat) {
    ...
    chat.MESSAGES.LAST_DELIVERED_MESSAGE_ID = chat.MESSAGES.MESSAGES[0].MESSAGE_ID;
    chat.MESSAGES.LAST_SEEN_MESSAGE_ID = chat.MESSAGES.MESSAGES[0].MESSAGE_ID;
    updateChatsTable();
}
...
function addMessage(author, message) {
    if (document.querySelector("#message" + message.MESSAGE_ID) != null) {
        cellClass = "myMessage";
        ...
        if (author.USER_ID != connectedUser.USER_ID) {        
        ...
        row.setAttribute("id", "message" + message.MESSAGE_ID);
    ...
    if (chatsArray.length > 0 && chatId != null) {
        ...
        if (chat.MESSAGES != null) {
            ...
            for (var i = chat.MESSAGES.MESSAGES.length - 1; i >= 0; i--) {
                ...
                author = await getUser(message.AUTHOR_ID);
            ...
            seenMess = await seenMessage(chat.MESSAGES.MESSAGES[0].MESSAGE_ID);
